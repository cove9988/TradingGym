...
class ForexTradingEnvV2(gym.Env):
    ...
    Parameters:
    state space: {start_cash, <owned_shares>, for s in symbols{<symbol.values>}, }
        ...
        information_columns (list(str)): Columns to use when building state space from the dataframe.
        out_of_cash_penalty (int, float): Penalty to apply if the algorithm runs out of cash

    action space: <forex_trading>
    ...
    def __init__(
        ...
        reward_scaling=1e-4,
        initial_amount=1e6,
        information_cols=["open", "close", "high", "low"],
        ...
        self.assets = df[self.symbol_col].unique()
        self.datetimes = df[datetime_col_name].sort_values().unique()
        self.random_start = random_start

        self.df = self.df.set_index(datetime_col_name)
        self.hmax = hmax
        ...
        self.state_space = (
            1 + len(self.assets) + len(self.assets) * len(self.information_cols)
        )
        ...
        if self.cache_indicator_data:
            print("caching data")
            self.cached_data = [
                self.get_datetime_vector(i) for i, _ in enumerate(self.datetimes)
            ]
            print("data cached!")
    ...
    def current_step(self):
        return self.datetimetime_index - self.starting_point

    def reset(self):
        self.seed()
        self.sum_trades = 0
        if self.random_start:
            starting_point = random.choice(range(int(len(self.datetimes) * 0.5)))
            self.starting_point = starting_point
        else:
            self.starting_point = 0
        self.datetime_index = self.starting_point
        self.episode += 1
        self.actions_memory = []
        ...
        init_state = np.array(
            [self.initial_amount]
            + [0] * len(self.assets)
            + self.get_datetime_vector(self.datetime_index)
        )
        self.state_memory.append(init_state)
        return init_state

    def get_datetime_vector(self, datetime, cols=None):
        if (cols is None) and (self.cached_data is not None):
            return self.cached_data[datetime]
        else:
            datetime = self.datetimes[datetime]
            if cols is None:
                cols = self.information_cols
            trunc_df = self.df.loc[datetime]
            v = []
            for a in self.assets:
                subset = trunc_df[trunc_df[self.symbol_col] == a]
                v += subset.loc[datetime, cols].tolist()
            assert len(v) == len(self.assets) * len(cols)
            return v

    def return_terminal(self, reason="Last datetime", reward=0):
    ...
    def step(self, actions):
        ...
        # if we're at the end
        if self.datetime_index == len(self.datetimes) - 1:
            ...
            closings = np.array(self.get_datetime_vector(self.datetime_index, cols=["close"]))
            ...
            state = (
                [coh] + list(holdings_updated) + self.get_datetime_vector(self.datetime_index)
            )
    ...
    def save_asset_memory(self):
        if self.current_step == 0:
            ...
            self.account_information["datetime"] = self.datetimes[
    ...
    def save_action_memory(self):
        if self.current_step == 0:
            ...
            return pd.DataFrame(
                {
                    "datetime": self.datetimes[-len(self.account_information["cash"]) :],
