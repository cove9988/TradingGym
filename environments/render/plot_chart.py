...
class TradingChart():
    ...
    def _render_net_worth(self, step_range, times, current_step, net_worths, benchmarks):
        ...
        # Plot Balances
        self.net_worth_ax.plot(times, net_worths[step_range], label='Balance', color="g")
        ...
        # Annotate the current Balance on the Balance graph
    ...
    def render(self, current_step, net_worths, benchmarks, trades, window_size=200):
        ...
        equity_percent = round((net_worth - initial_net_worth) / initial_net_worth * 100, 2)

        self.fig.suptitle('Balance: $' + str(net_worth) + ' | Equity: ' + str(equity_percent) + '%')

        window_start = max(current_step - window_size, 0)
        step_range = slice(window_start, current_step + 1)
        times = self.df['time'].values[step_range]
        ...
        # Hide duplicate Balance date labels
